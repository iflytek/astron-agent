const translation = {
  ...
  promptDebugger: {
    ...
    importFailed: '导入失败',
    importResponseInvalid: '导入结果无效，请重试',
    importReportTitle: '导入依赖检查',
    importReportTotal: '依赖总数',
    importReportMapped: '已匹配',
    importReportAmbiguous: '存在歧义',
    importReportUnresolved: '未解析',
    importReportUnknown: '待确认',
    importReportWarning: '部分节点需要处理',
    importReportWarningDescription:
      '工作流已保存为草稿。请在画布中补充或重新绑定未解析的依赖后再调试或发布。',
    importReportSuccess: '依赖检查完成',
    importReportSuccessDescription: '所有依赖均已匹配，可以进入画布继续配置。',
    importReportOpenCanvas: '查看并处理',
    importReportUnknownNode: '未知节点',
    importReportNoReason: '目标环境中未找到可用依赖',
    importReportMappingType: '匹配方式：',
    importReportReason: '原因：',
    importReportPluginId: '插件 ID：',
    importReportDatabaseId: '数据库 ID：',
    importReportWorkflowId: '工作流 ID：',
    importReportKnowledgeId: '知识库 ID：',
    importReportResourceId: '资源 ID：',
    importReportOperationId: '操作 ID：',
    importReportVersion: '版本：',
    importReportCandidatePluginIds: '候选插件 ID：',
    importReportNotProvided: '未提供',
    importMappingType_SOURCE_ID: '源 ID 精确匹配',
    importMappingType_COMPATIBLE_NAME: '兼容名称匹配',
    importDependencyExecutionBlocked:
      '工作流存在未解析的导入依赖，请重新绑定后再调试。',
    importDependencyPublishBlocked:
      '工作流存在未解析的导入依赖，请重新绑定并调试通过后再发布。',
    nodeDebugCancelled: '节点调试已取消',
    nodeDebugRequestFailed: '节点调试请求失败，请稍后重试',
    importReasonDatabaseMissing: '目标空间中没有可用的数据库',
    importReasonContractIncompatible: '插件操作或参数结构不兼容',
    importReasonMultipleVersions: '源插件 ID 对应多个可用版本',
    importReasonDuplicateName: '目标空间中存在多个同名插件',
    importReasonSameNameIncompatible: '同名插件的参数结构不兼容',
    importReasonToolMissing: '目标空间中未找到或无权使用该插件',
    importReasonMultipleTools: '目标空间中存在多个相同标识的插件',
    importReasonWorkflowMissing: '目标空间中没有可用的子工作流',
    importReasonKnowledgeMissing: '目标空间中没有可用的知识库',
    importReasonKnowledgeItemsMissing: '一个或多个知识库在目标空间中不可用',
    importReasonCode_SOURCE_ID_MATCHED: '已按源插件 ID 匹配',
    importReasonCode_UNIQUE_COMPATIBLE_NAME_MATCHED: '已按唯一同名兼容插件匹配',
    importReasonCode_TOOL_MISSING: '目标空间中未找到或无权使用该插件',
    importReasonCode_CONTRACT_INCOMPATIBLE: '插件操作或参数结构不兼容',
    importReasonCode_SAME_NAME_CONTRACT_INCOMPATIBLE:
      '同名插件的参数结构不兼容',
    importReasonCode_MULTIPLE_COMPATIBLE_TOOLS:
      '目标空间中存在多个同名兼容插件',
    importReasonCode_MULTIPLE_TOOL_VERSIONS: '存在多个无法唯一确定的插件版本',
    importReasonCode_DATABASE_MISSING: '目标空间中没有可用的数据库',
    importReasonCode_WORKFLOW_MISSING: '目标空间中没有可用的子工作流',
    importReasonCode_KNOWLEDGE_MISSING: '目标空间中没有可用的知识库',
